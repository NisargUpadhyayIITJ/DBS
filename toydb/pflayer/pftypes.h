...
/* Statistics collected by PF buffer manager */
typedef struct PFstats {
	int logical_reads;   /* number of logical read requests (Get) */
	int logical_writes;  /* number of logical writes (Unfix with dirty) */
	int phys_reads;      /* physical disk reads */
	int phys_writes;     /* physical disk writes */
	int page_hits;       /* buffer hits */
	int page_misses;     /* buffer misses */
} PFstats;
...
/* prototypes matching implementations in buf.c */
